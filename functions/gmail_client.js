// functions/gmail_client.js
//
// Lightweight Gmail API wrapper for ReachPoint.
// Exposes:
//   - gmailClient.ensureInit()
//   - gmailClient.send({ to, subject, html })
//   - gmailClient.sendBulk({ to, subject, html, onProgress? })
//
...
export function ensureInit() {
  ...
  initPromise = new Promise((resolve, reject) => {
    if (!window.gapi) {
      return reject(
        new Error(
          'Google API (gapi) not loaded. Ensure <script src="https://apis.google.com/js/api.js"></script> is added to index.html.'
        )
      );
    }
...
export async function sendBulk({ to, subject, html, onProgress }) {
  ...
  for (let i = 0; i < uniq.length; i++) {
    const addr = uniq[i];
    try {
      const res = await send({ to: addr, subject, html });
...
// Optional: also attach to window for any legacy code
if (typeof window !== 'undefined') {
  window.gmailClient = gmailClient;
}

// Also provide a default export in case you ever use:
